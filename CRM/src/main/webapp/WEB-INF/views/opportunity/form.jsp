...
<!-- 选择营销活动窗口 -->
<div class="modal fade" id="selectCampaignModal" tabindex="-1"
	department="dialog" aria-hidden="true">
	<div class="modal-dialog modal-product">
...
<div class="widget">
	...
	<div id="opportunityPanel" role="content">
		<div class="widget-body no-padding">
			<form method="post" id="opportunityForm" name="opportunity"
				class="smart-form" onSubmit="return false;">
				<input type="hidden" name="id" value="${opportunityModel.id }" /> <input
					name="createTime" type="hidden"
					value="${opportunityModel.createTime }" />
				<fieldset>
					<div class="row">
						<section class="col col-md-6">
							<label class="label">名称</label> <label class="input"> <input
								name="name" value="${opportunityModel.name }" type="text"
								data-rule="required;">
							</label>
						</section>
						<section class="col col-md-6">
							<label class="label">销售机会编号</label> <label class="input">
								<input type="text" name="code" value="${opportunityModel.code }"
								placeholder="自动生成销售机会编号" readonly="readonly">
							</label>
						</section>
					</div>
					...
					<div class="row">
						<section class="col col-md-6">
							<label class="label">负责人</label> <label class="select"> <input
								type="hidden" id="userCode" value="${opportunityModel.user.id }">
						...
						<section class="col col-md-6">
							<label class="label">合计</label> <label class="input"> <input
								type="text" name="total" value="${opportunityModel.total }"
					...
					<div class="row">
						<section class="col col-md-6">
							<label class="label">选择客户</label> <label class="input"> <input
								...
								value="${opportunityModel.customer.id }" type="hidden" /> <input
								class="customer-select" name="customerName" readonly="readonly"
								value="${opportunityModel.customer.name }" type="text"
								data-rule="required" placeholder="点击选择客户" />
							</label>
						</section>
						<section class="col col-md-6">
							<label class="label">来自营销活动</label> <label class="input">
								<input class="campaign-id" name="campaign.id"
								value="${opportunityModel.campaign.id }" type="hidden" /> <input
								class="campaign-select" name="campaignName" readonly="readonly"
								value="${opportunityModel.campaign.name }" type="text"
								data-rule="required" placeholder="点击选择营销活动" />
							</label>
						</section>
					</div>
					...
					<div class="row">
						<section class="col col-md-6">
							<label class="label">销售机会来源</label> <label class="select">
								<input type="hidden" id="resourceCode"
								value="${opportunityModel.resource.code }"> <select
								name="resource.code" id="resource">
						...
						<section class="col col-md-6">
							<label class="label">销售阶段</label> <label class="select">
								<input type="hidden" id="statusCode"
								value="${opportunityModel.status.code }"> <select
					...
					<div class="row">
						<section class="col col-md-6">
							<label class="label">可能性(%)</label> <label class="input">
								...
								value="${opportunityModel.probability }"
								data-rule="required; integer; range[0~2147483647];">
							</label>
						</section>
						<section class="col col-md-6">
							<label class="label">预计结束日期:</label> <label class="input">
								...
								onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" name="endDate"
								value="${opportunityModel.endDate }" data-rule="required">
							</label>
						</section>
					</div>
					...
					<div class="row">
						<section class="col col-md-12">
							<label class="label">描述</label> <label class="input"> <textarea
									...
									maxlength="1500">${opportunityModel.description }</textarea>
							</label>
						</section>
					</div>
				</fieldset>
				<footer>
					<button type="submit" name="submit"
						class="save-opportunity-btn btn btn-primary">保存</button>
