...
<div class="panel-body">
	<form id="menu_form" name="menu" role="form" onSubmit="return false;">
		<input id="menuId" type="hidden" name="id" value="${menu.id }" /> <input
			id="icon" type="hidden" name="icon" value="${menu.icon }" /> <input
			id="parentId" type="hidden" name="parent.id" value="${menuId }" />

		<div class="row">
			<div class="form-group col-md-6">
				<label for="menuName">菜单名称（必填）：</label> <input
					class="form-control validate[required]" name="name"
					value="${menu.name}" data-rule="required" placeholder="菜单名称..."
					maxlength="50" />
			</div>
		</div>
		...
		<div class="row">
			<div class="form-group col-md-6">
				<label for="menuUrl">菜单URL（必填）：</label> <input
					class="form-control validate[required]" name="url"
					value="${menu.url}" placeholder="菜单URL..." maxlength="50" />
			</div>

			<div class="col-md-6 table-form-item">
				<label for="menuIcon">菜单图标:</label> <a href="menu/selectMenuIcon"
					data-target="#menuIconForm" data-toggle="modal" class="menuIcon"><i
					id="menuIconView" class="${menu.icon }">选择图标</i> </a>
			</div>
		</div>

		<div class="row">
			<div class="modal fade" id="menuIconForm" tabindex="-1" role="dialog"
				aria-hidden="true">
		...
		<div class="row">
			...
			<div class="col-md-12 text-center">
				...
				<button type="button" class="add-resource btn btn-info"
					style="display: none;" data-id="${menu.id}">添加资源</button>
			</div>
		</div>
